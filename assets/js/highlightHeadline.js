...
function initHighlightHeadline() {
  ...
  const headings = Array.from(
    document.querySelectorAll(".content_with_heading :is(h1, h2, h3, h4)"),
  );
  ...
  const getTocLinkFromHeading = (h) => {
    return document.querySelector(
      `.toc a[href="${windowPath}#${encodeURIComponent(h.id).replace(/%\w\w/g, (match) => match.toLowerCase())}"]`,
    );
  };
  ...
  function beginObservation(docHeight) {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const lastVisible = Array.from(visibleHeadings.values()).sort(
