---
draft: false
title: "ŽFBH"
country:
  - "bosnia-and-herzegovina"
operator: "žfbh"
---

Die Željeznice Federacije Bosne i Hercegovine (ŽFBH) ist das Eisenbahnverkehrsunternehmen der Förderation Bosnien und Herzegowina im gleichnamigen Land. Neben ihr existiert in dem Land auch noch die [Željeznice Republike Srpske (ŽRS)](/operator/zrs) der Förderation Republika Srpska. Die ŽFBH betreibt auf ihrem 608 km großen Schienennetz nur sehr wenige Verbindungen und auch nur auf einem Teil ihres Netzes.

## Zusammenfassung

- Nur FIP ermäßigte Tickets werden akzeptiert, es gibt keine Freifahrtscheine.
- Der Verkauf von FIP-Tickets findet nur vor Ort statt und ist nicht immer sichergestellt.

## Gültigkeit FIP Tickets

{{< fip-validity type="fip-coupon" status="invalid" >}}
{{< fip-validity type="fip-reduced-ticket" status="valid" >}}

FIP 50 Tickets sind auf Verbindungen der ŽFBH gültig. Bei grenzüberschreitenden Fahrten nach Kroatien muss entweder ein durchgängiges FIP 50 Ticket oder ein FIP 50 Ticket bis zur kroatischen Grenze und ein FIP Freifahrtschein der HŽ vorhanden sein. Bei Fahrten über die Entitätsgrenze (derzeit nicht vorhanden) wäre entweder ein durchgängiges FIP 50 Ticket oder ein FIP 50 Ticket bis zur Entitätsgrenze und ein FIP Freifahrtschein der ŽRS nötig.

## Zugkategorien und Reservierungen

{{% highlight important %}}
Die aktuell verkehrenden Züge und deren Einschränkungen auf bestimmte Verkehrstage sind auf der [ŽFBH Website](https://www.zfbh.ba/en/time-table/) zu finden. Die Zugkategorie "Brzi" wird hier mit "Fast train" und "Putnički" mit "Local train" übersetzt.
{{% /highlight %}}

{{% train-category
    id="b"
    title="Brzi (B)"
    type="highspeed"
    fip_accepted=true
    reservation_required=false
    reservation_possible=unknown
%}}

Nationale und nach Kroatien verkehrende internationale Schnellzüge, die mit wenig Zwischenhalten vergleichsweise schnell innerhalb der Entität Bosnien und Herzegowina und ins kroatische Ploče verkehren. Auf der Verbindung zwischen Sarajevo - Mostar - Čapljina (- Ploče) kommen moderne Talgo-Wagen zum Einsatz, auf der anderen Strecke Sarajevo - Maglaj sind deutlich ältere Personenwagen möglich.

#### Reservierung

Uns ist aktuell nicht bekannt, ob Reservierungen angeboten werden und ob diese verpflichtend sind. Da die Tickets jedoch nur personenbedient verkauft werden, besteht keine Gefahr hier versehentlich keine Reservierung zu haben. 
{{% /train-category %}}

{{% train-category
    id="p"
    title="Putnički (P)"
    type="regional"
    fip_accepted=true
    reservation_required=false
%}}
Regionalzüge, die kurze Strecken rund um die Hauptstadt Sarajevo mit allen Zwischenhalten bedienen.
{{% /train-category %}}

## Ticket- und Reservierungskauf

### Vor Ort

{{% booking id="zfbh-ticket-office" /%}}

### Im Zug

Nationale FIP Tickets können nur im Zug erworben werden, wenn der Einstieg an einem Bahnhof ohne oder mit geschlossenem Fahrkartenschalter erfolgt. Alternativ gilt dies auch, wenn die Einreise aus einem anderen Land erfolgt und die Weiterfahrt im gleichen Zug erfolgt.[^1]

## Ermäßigungen

Kinder bis einschließlich 3 Jahren reisen kostenlos, wenn sie keinen extra Sitzplatz benötigen. Kinder unter 12 Jahre erhalten unabhängig von FIP 50 % Rabatt auf den regulären Fahrpreis.[^2] Eine Übersicht über alle Ermäßigungen außerhalb von FIP ist auf der [ŽFBH Website](https://www.zfbh.ba/en/putnicki-prijevoz/komercijalne-povlastice/#1499947770755-76ae7c80-fad0) zu finden.

Für Reisende durch mehrere Balkanländer könnte sich der Balkan Flexipass preislich lohnen. Er ermöglicht unbegrenzte Fahrten auf den Eisenbahnnetzen von:

- Bosnien und Herzegowina (ŽFBH, ŽRS),
- Bulgarien (BDŽ),
- Nordmazedonien (MŽ-Transport),
- Griechenland (HT & Attica),
- Montenegro (ŽPCG),
- Rumänien (CFR Calatori & Regiotrans),
- Serbien (SV),
- Türkei (TCDD).

[Weitere Informationen auf der ŽPCG-Website](https://zpcg.me/en/medjunarodni-prevoz/povlastice)

## Tarifliche Besonderheiten

### Schienenersatzverkehr

In Bussen des Schienenersatzverkehrs gelten FIP Vergünstigungen, wenn sie einen Zug ersetzen, in dem FIP gültig gewesen wäre.[^1]

## Empfehlungen

{{% highlight tip %}}
Die ŽFBH betreibt nur sehr wenige Züge auf nur noch einzelnen Strecken der Entität Bosnien und Herzegowina. Ein Highlight ist sicherlich die Fahrt in den Talgo-Wagen auf der Strecke zwischen Sarajevo und Mostar, die zu den schönsten Bahnstrecken Europas gehört. Damit sind die zwei touristischsten Städte des Landes zumindest etwa drei mal am Tag per Bahn verbunden. Auch die Weiterfahrt ins kroatische Ploče bietet sich an, der Ort ist nur aus Bosnien und Herzegowina per Bahn erreichbar und nicht mit dem restlichen kroatischen Schiennenetz verbunden.

Leider werden derzeit keine entitätsüberschreitenden Züge angeboten und auch die Taktung innerhalb der Entität Bosnien und Herzegowina ist sehr schlecht. Beispielsweise hat der Regionalzug zwischen Sarajevo und Zenica keinerlei Anschluss an den Regionalzug nach Maglaj.
{{% /highlight %}}

## Quellen

[^1]: [Rail Delivery Group](https://www.raildeliverygroup.com/rst/europe-and-fip/countries/469782241-bosnia-herzegovina.html#ZFBH)

[^2]: [ŽFBH Ermäßigungsübersicht](https://www.zfbh.ba/en/putnicki-prijevoz/komercijalne-povlastice/#1499947770755-76ae7c80-fad0)
