---
draft: false
title: "KMŁ"
country:
  - "poland"
operator: "kml"
---

Die Koleje Małopolskie, kurz KMŁ, ist ein polnisches Eisenbahnverkehrsunternehmen, das hauptsächlich Regionalverkehr in der Woiwodschaft Kleinpolen anbietet. Es ist eines der insgesamt fünf verschiedenen Unternehmen, das in [Polen](/country/poland) FIP anbietet.

Auf der Website bietet die KMŁ eine [Übersicht der betriebenen Linien](https://kolejemalopolskie.com.pl/pl/rozklad-jazdy/rozklady-kolejowe) an. Dabei ist zu beachten, dass die Strecken teilweise auch durch andere Betreiber parallel bedient werden.

## Zusammenfassung

- KMŁ akzeptiert FIP Freifahrt und FIP 50 Tickets.
- Nur 2. Klasse in den Zügen vorhanden und keine Reservierungen möglich.

## Gültigkeit FIP-Tickets

{{< fip-validity type="fip-coupon" status="valid" >}}
{{< fip-validity type="fip-reduced-ticket" status="valid" >}}

FIP Freifahrtscheine und FIP 50 Tickets sind bei der KMŁ gültig.

Bei Fahrten mit verschiedenen Betreibern innerhalb Polens muss entweder ein durchgängiges FIP 50 Ticket oder FIP Freifahrtscheine beider Betreiber vorhanden sein.

## Zugkategorien und Reservierungen

Die KMŁ unterscheidet nicht in unterschiedliche Zugkategorien. Die Züge haben in der Verbindungsauskunft am Anfang das Kürzel "KMŁ" und im Anschluss eine konkrete Zugnummer. Liniennummern sind zwar vorhanden, werden aber in der Verbindungsauskunft meist nicht angezeigt. Es werden nur Tickets der 2. Klasse angeboten, da es in KMŁ-Zügen keine 1. Klasse gibt. Zudem besteht keine Reservierungsmöglichkeit.

Ob auch in den Bussen der KMŁ FIP-Fahrkarten möglich sind, ist uns aktuell nicht bekannt. Im Schienenersatzverkehr sollten sie aber wie bei anderen Betreibern auch akzeptiert werden.

## Ticket- und Reservierungskauf

{{% highlight important %}}
Da die KMŁ erst seit dem 01.04.2026 FIP-Mitglied ist sind die Buchungsmöglichkeiten derzeit noch sehr eingeschränkt und werden voraussichtlich erst nach und nach weiter ergänzt.
{{% /highlight %}}

### Vor Ort

{{% booking id="kml-ticket-office"/%}}

### Im Zug

FIP 50 Fahrkarten müssen bei Einstieg an Bahnhöfen, an denen der Ticketschalter geöffnet ist, bereits vor Fahrtantritt erworben werden.[^1] Gibt es keinen geöffneten Ticketschalter, können die Tickets auch direkt im Zug erworben werden. Dafür muss nach dem Einstieg das Zugbegleitpersonal aufgesucht werden. Die Tickets können in bar oder mit kontaktloser Kreditkarte bezahlt werden. Eine Zahlung ist nur in der Landeswährung Złoty möglich.

## Ermäßigungen

Uns sind derzeit noch keine Informationen über Ermäßigungen bekannt.

## Empfehlungen

{{% highlight tip %}}
Das noch neue FIP-Mitglied KMŁ ist eine gute Ergänzung für die Fahrten im polnischen Streckennetz. Auch wenn noch nicht allzu viel über die FIP-Nutzung bekannt ist, dürfte sie ähnlich wie bei den anderen regionalen Bahngesellschaften im Land einfach und bequem möglich sein. Auf einigen Strecken verkehren auch parallel Züge von PKP InterCity und Polregio, wodurch bei der Nutzung eines Freifahrtscheins ein zusätzlicher Freifahrtschein der PKP sehr hilfreich ist, um bei der Zugauswahl nicht zu eingeschränkt zu sein.

Reisehighlights mit der KMŁ sind u. a. zum bekannten Freizeitpark Energylandia (Bahnhof Zator Park Rozrywki) oder zum Salzbergwerk Wieliczka (Bahnhof Wieliczka-Rynek-Kopalnia).
{{% /highlight %}}

## Quellen

[^1]: [Rail Delivery Group](https://www.raildeliverygroup.com/rst/europe-and-fip.html)
